...
class GitHubRepository:
    ...
    def get_chart_content(
        self, chart_file_path: str
    ) -> Tuple[ContentFile, Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"Chart.yaml not found at path '{chart_file_path}' "
                f"in repository '{self.repo_path}': {e}"
            )
            sys.exit(1)

    def update_chart_file(
        self, content: ContentFile, commit_message: str, updated_content: str
    ) -> None:
        """Update Chart.yaml file in the repository."""
        try:
            self.repo.update_file(
                content.path, commit_message, updated_content, content.sha
            )
        except Exception as e:
